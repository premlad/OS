...
namespace The_PIT_Archive.Data_Access_Layer
{
    internal class MasterClass
    {
        ...
        public SqlCeConnection con = new SqlCeConnection(CryptographyHelper.Decrypt(SESSIONKEYS.CONNECT));
        ...
        public void SAVEDBTEXTLOG(string DbPath)
        {
            try
            {
                RegistryKey key = Registry.CurrentUser.CreateSubKey(@"The-PIT-Archive\OurSettings");
                key.SetValue("PATH", DbPath);
                key.Close();
                //string directory = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86);
                //if (File.Exists(Path.Combine(directory, "The PIT Archive", "The-PIT-Archive-DB.txt")))
                //{
                //	File.Delete(Path.Combine(directory, "The PIT Archive", "The-PIT-Archive-DB.txt"));
                //}
                //using (FileStream fs = new FileStream((Path.Combine(directory, "The PIT Archive", "The-PIT-Archive-DB.txt")), FileMode.OpenOrCreate, FileAccess.Write))
                //{
                //	StreamWriter m_streamWriter = new StreamWriter(fs);
                //	// Write to the file using StreamWriter class
                //	m_streamWriter.BaseStream.Seek(0, SeekOrigin.End);
                //	m_streamWriter.Write(CryptographyHelper.Encrypt(DbPath));
        ...
        public string GETDBTEXTLOG()
        {
            try
            {
                string val = "";
                RegistryKey key = Registry.CurrentUser.OpenSubKey(@"The-PIT-Archive\OurSettings");
                if (key != null)
                {
                    val = key.GetValue("PATH").ToString();
                    key.Close();
                }
                //string directory = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86);
                //FileStream fs = new FileStream(Path.Combine(directory, "The PIT Archive", "The-PIT-Archive-DB.txt"), FileMode.OpenOrCreate, FileAccess.Write);
                //StreamReader ss = new StreamReader(fs);
                //string val = CryptographyHelper.Decrypt(ss.ReadLine());
                //ss.Close();
                //ss.Dispose();
                return val;
            }
        ...
        public string GETLOGINDATAOFFIUCER()
        {
            DataSet ds = new MasterClass().getDataSet("SELECT TOP 1 * FROM M_LOG_AUDIT WHERE ENTEREDBY = '2' ORDER BY ENTEREDON DESC");
            List<string> termsList = new List<string>();
            if (ds.Tables[0].Rows.Count > 0)
            {
                string a = CryptographyHelper.Decrypt(ds.Tables[0].Rows[0]["NAME"].ToString());
                if (a.Trim() == "LOG OUT")
                {
                    return "Y";
                }
